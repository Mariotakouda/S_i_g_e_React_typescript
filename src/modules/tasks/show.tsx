...
export default function TaskShow() {
  ...
  if (loading) return (
    <div className="d-flex flex-column justify-content-center align-items-center min-vh-100 bg-white">
      ...
      <span className="text-muted fw-medium">Chargement de la mission...</span>
    </div>
  );
      ...
      <div className="container" style={{ maxWidth: "1000px" }}>
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <button 
            ...
            className="btn btn-link text-decoration-none text-secondary p-0 d-flex align-items-center gap-2 fw-semibold border-0"
          >
          ...
          <div className="d-flex gap-2">
            {isAdmin && (
              <button 
                ...
                className="btn btn-white border shadow-sm px-4 py-2 d-flex align-items-center gap-2 fw-bold text-dark"
          ...
          <div className="card-body p-4 p-md-5">
            ...
            <div className="row align-items-start mb-5">
              <div className="col-lg-8">
                ...
                <p className="text-muted mb-0">Référence interne : <span className="text-dark fw-medium">#{task.id}</span></p>
              </div>
            </div>

            {/* Grid des Métadonnées (Look Professionnel) */}
            <div className="row g-4 mb-5 pb-5 border-bottom">
              ...
              <div className="col-md-4">
                <div className="d-flex align-items-center gap-3">
                  ...
                  <div>
                    <div className="text-muted small fw-bold text-uppercase">Date d'échéance</div>
            ...
            <div className="mb-5">
              ...
              <div className="p-4 rounded-4 bg-light border-0" style={{ whiteSpace: "pre-wrap", lineHeight: "1.8", color: "#4B5563" }}>
                {task.description || "Aucun détail complémentaire pour cette mission."}
              </div>
            </div>

            {/* Section Documents (Cartes de téléchargement) */}
            <h5 className="fw-bold text-dark mb-3">Livrables et Documents</h5>
              ...
              <div className="col-md-6">
                <div className="card h-100 border bg-white shadow-none rounded-4">
                  <div className="card-body d-flex flex-column">
                    ...
                    <h6 className="fw-bold mb-1">Consignes initiales</h6>
                    <p className="small text-muted mb-4">Document de référence pour la mission.</p>
                    
                    {task.task_file ? (
                      <button
                        onClick={() => handleDownloadPDF(task.task_file!, 'consignes.pdf')}
                      ...
                      <div className="alert alert-light border-0 small mb-0 mt-auto py-2">Non disponible</div>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className={`card h-100 border shadow-none rounded-4 ${task.report_file ? 'border-success-subtle bg-success-subtle' : 'bg-white'}`}>
                  <div className="card-body d-flex flex-column">
                    <div className="d-flex justify-content-between align-items-start mb-3">
                      ...
                      <span className="small text-muted fw-bold text-uppercase">{task.report_file ? 'Terminé' : 'Manquant'}</span>
                    </div>
                    <h6 className="fw-bold mb-1 text-dark">Rapport de fin de mission</h6>
                    <p className="small text-muted mb-4 text-dark opacity-75">Document de clôture soumis par l'employé.</p>
                    
                    {task.report_file ? (
                      <button
                        onClick={() => handleDownloadPDF(task.report_file!, 'rapport.pdf')}
                      ...
                      <div className="bg-light rounded-3 text-center text-muted small py-2 mt-auto fw-medium border">
                        {task.status === 'completed' ? 'Rapport non soumis' : 'En attente de réalisation'}
                      </div>
                    )}
                  </div>
                </div>
        ...
        {/* Footer Administratif (Optionnel) */}
        <div className="text-center mt-5">
          <p className="text-muted small italic">Toute modification effectuée sur cette mission sera tracée dans le journal d'audit.</p>
        </div>
      </div>
    </div>
  );
