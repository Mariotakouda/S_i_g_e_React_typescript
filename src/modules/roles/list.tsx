...
const Icon = ({ name, onClick }: { name: 'eye' | 'edit' | 'trash' | 'plus' | 'search', onClick?: () => void }) => {
  const icons = {
  ...
  return (
    <span className={`icon-wrapper ic-${name}`} onClick={onClick}>
      {icons[name]}
    </span>
  );
};
        ...
        <div className="card-body p-2">
          <form onSubmit={handleSearch} className="row g-2 text-center text-md-start">
            ...
            <div className="col-12 col-md-auto d-flex gap-2 p-1">
              <button type="submit" className="btn btn-dark px-4 fw-bold flex-grow-1 shadow-sm" disabled={loading}>Filtrer</button>
              {search && <button type="button" onClick={handleClearSearch} className="btn btn-outline-secondary px-3 border-0">✕</button>}
            </div>
          </form>
        </div>
      </div>

      {error && <div className="alert alert-danger border-0 shadow-sm py-3 small">{error}</div>}

      {/* Contenu principal */}
      {loading && roles.length === 0 ? (
        <div className="text-center py-5"><div className="spinner-border text-primary"></div></div>
      ) : roles.length === 0 ? (
        <div className="text-center py-5 bg-white rounded-4 shadow-sm text-muted">Aucun rôle trouvé.</div>
      ) : (
        <>
          {/* VUE MOBILE */}
              ...
              <div key={r.id} className="card border-0 shadow-sm mb-3 rounded-4">
                <div className="card-body p-3">
                  <div className="d-flex justify-content-between align-items-start mb-3">
                    <h6 className="fw-bold text-dark mb-0" style={{ fontSize: '16px' }}>{r.name}</h6>
                    <span className="badge bg-primary-subtle text-primary rounded-pill px-3 py-2" style={{ fontSize: '11px' }}>
                      {r.employees_count ?? 0} pers.
                    </span>
                  </div>
                  <div className="text-muted mb-3" style={{ fontSize: '13px' }}>Créé le {formatDate(r.created_at)}</div>
                  <div className="d-flex gap-2">
                    <Link to={`${r.id}`} className="btn btn-white flex-grow-1 border shadow-sm d-flex align-items-center justify-content-center py-2 px-3">
                      <Icon name="eye" /><span className="ms-2 fw-bold" style={{ fontSize: '13px' }}>Détails</span>
                    </Link>
                    <Link to={`${r.id}/edit`} className="btn btn-white border shadow-sm px-3 text-warning d-flex align-items-center"><Icon name="edit" /></Link>
                    <button onClick={() => remove(r.id, r.name)} className="btn btn-white border shadow-sm px-3 text-danger d-flex align-items-center"><Icon name="trash" /></button>
                  </div>
                </div>
              </div>
            ))}
              ...
              <table className="table table-hover align-middle mb-0">
                <thead className="bg-light">
                  <tr style={{ fontSize: '12px', textTransform: 'uppercase', color: '#666', letterSpacing: '0.8px' }}>
                    <th className="px-4 py-3">Intitulé du rôle</th>
                ...
                <tbody style={{ fontSize: '15px' }}>
                  {roles.map((r) => (
                    <tr key={r.id}>
                      <td className="px-4 py-4 fw-bold text-dark" style={{ fontSize: '16px' }}>{r.name}</td>
                      <td className="py-4 text-center">
                        <span className="fw-bold px-3 py-2 bg-light rounded-pill text-primary" style={{ fontSize: '13px' }}>
                          {r.employees_count ?? 0} employés
                        </span>
                      </td>
                      <td className="py-4 text-muted">{formatDate(r.created_at)}</td>
                      <td className="text-center px-4">
                        <div className="d-inline-flex gap-2">
                          <Link to={`${r.id}`} className="btn btn-outline-primary rounded-3 p-2 d-flex shadow-sm"><Icon name="eye" /></Link>
                          <Link to={`${r.id}/edit`} className="btn btn-outline-warning rounded-3 p-2 d-flex shadow-sm"><Icon name="edit" /></Link>
                          <button onClick={() => remove(r.id, r.name)} className="btn btn-outline-danger rounded-3 p-2 d-flex shadow-sm"><Icon name="trash" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          {last_page > 1 && (
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mt-4 gap-3">
              ...
              <div className="d-flex gap-2 w-100 w-md-auto">
                <button className="btn btn-white border px-4 py-2 fw-bold shadow-sm rounded-3" onClick={() => setPage(page - 1)} disabled={loading || current_page === 1}>Précédent</button>
                <button className="btn btn-white border px-4 py-2 fw-bold shadow-sm rounded-3" onClick={() => setPage(page + 1)} disabled={loading || current_page === last_page}>Suivant</button>
              </div>
            </div>
          )}
        </>
