...
// --- SKELETON & ICONS (Gardés tels quels pour votre design) ---
const StatSkeleton = () => (
...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState({ emp: 0, dep: 0, task: 0, leave: 0, managers: 0, roles: 0, announcements: 0, presences: 0 });
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      setStats(stats);
      setRecentEmployees(recent.employees);
      setRecentAnnouncements(recent.announcements);
      setRecentPresences(recent.presences);
    ...
    return (
      <div className="admin-dashboard-container">
        <div className="error-alert-box">
          <Icons.Alert />
          <p>{error}</p>
          <button onClick={loadDashboardData} className="btn-retry-modern">Actualiser</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="admin-dashboard-container">
      <header className="dashboard-header">
        ...
        <p className="dashboard-welcome">Bonjour, <strong>{user?.name || 'Administrateur'}</strong></p>
      </header>
      ...
      <div className="dashboard-recent-grid">
        <Section title="Employés récents" link="/admin/employees" loading={loading} data={recentEmployees}>
        ...
        <Section title="Présences" link="/admin/presences" loading={loading} data={recentPresences}>
        ...
        <Section title="Annonces" link="/admin/announcements" loading={loading} data={recentAnnouncements}>
          {recentAnnouncements.map(a => (
            <div key={a.id} className="item-row">
              <div className="item-info">
                <span className="item-main">{a.title}</span>
                <span className="item-sub">{a.message?.substring(0, 35)}...</span>
              </div>
            </div>
          ))}
        </Section>
...
// Composants internes (Card et Section)
  ...
  return (
    <div className="stat-card-premium">
      ...
      <div className="stat-details">
        ...
        <Link to={link} className="stat-link">Voir détails</Link>
      </div>
    </div>
  );
}

function Section({ title, children, link, loading, data }: any) {
  return (
