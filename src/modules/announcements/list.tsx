...
const SkeletonRow = () => (
  <tr className="skeleton-row">
    <td className="px-4 py-4">
      <div
        className="skeleton"
        style={{ width: "80px", height: "24px", borderRadius: "20px" }}
      ></div>
    </td>
    <td className="py-4">
      <div
...
export default function AnnouncementList() {
  ...
  const canManage =
    user?.role === "admin" || managerStatus?.is_manager || false;
  ...
  async function load() {
    try {
      ...
      // On vérifie où se trouvent les données (dépend de votre service.ts)
    ...
    <div className="container-fluid py-4 py-md-5 px-3 px-md-5 bg-light min-vh-100">
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        ...
        {canManage && (
          <Link
            to="create"
            className="btn btn-primary d-inline-flex align-items-center shadow-sm px-4 py-2 rounded-3"
          >
            <Icon name="plus" />{" "}
      ...
      {error && (
        <div className="alert alert-danger border-0 py-3 small">{error}</div>
      )}
              ...
              >
                <div className="card-body p-3">
                  <div className="d-flex justify-content-between align-items-start mb-2">
                    {a.is_general ? (
                      <span
                        className="badge rounded-pill px-3 py-2 bg-info"
                        style={{ fontSize: "11px" }}
                      >
                        Générale
                      </span>
                      ...
                      <span
                        className="badge rounded-pill px-3 py-2 bg-primary"
                        style={{ fontSize: "11px" }}
                      >
                        Dépt: {a.department.name}
                      </span>
                    ) : a.employee ? (
                      <span
                        className="badge rounded-pill px-3 py-2 bg-warning text-dark"
                        style={{ fontSize: "11px" }}
                      >
                        Pour: {a.employee.first_name} {a.employee.last_name}
                      </span>
                    ) : (
                      <span
                        className="badge rounded-pill px-3 py-2 bg-secondary"
                        style={{ fontSize: "11px" }}
                      >
                        Individuelle
                      </span>
                  ...
                  <Link
                    to={`${a.id}`}
                    className="btn btn-white btn-sm w-100 shadow-sm py-2 d-flex align-items-center justify-content-center"
                  >
                    <Icon name="eye" />
                    <span className="ms-2 fw-bold">Détails</span>
                  </Link>
                </div>
              </div>
            ))}
                ...
                : announcements.map((a) => (
                    <tr key={a.id}>
                      <td className="px-4 py-4">
                        {a.is_general ? (
                          <span
                            className="badge fw-bold px-3 py-2 rounded-pill bg-info"
                            style={{ fontSize: "10px" }}
                          >
                            GÉNÉRALE
                          </span>
                          ...
                          <span
                            className="badge fw-bold px-3 py-2 rounded-pill bg-primary"
                            style={{ fontSize: "10px" }}
                          >
                            DÉPT: {a.department.name.toUpperCase()}
                          </span>
                        ) : a.employee ? (
                          <span
                            className="badge fw-bold px-3 py-2 rounded-pill bg-warning text-dark"
                            style={{ fontSize: "10px" }}
                          >
                            POUR: {a.employee.first_name.toUpperCase()}{" "}
                            {a.employee.last_name.toUpperCase()}
                          </span>
                        ) : (
                          <span
                            className="badge fw-bold px-3 py-2 rounded-pill bg-secondary"
                            style={{ fontSize: "10px" }}
                          >
                            INDIVIDUELLE
                          </span>
                      ...
                      <td className="text-center px-4">
                        <div className="d-inline-flex gap-2">
                          <Link
                            to={`${a.id}`}
                            className="btn btn-outline-primary btn-sm rounded-3 p-2 d-flex shadow-sm"
                          >
                            <Icon name="eye" />
                          </Link>
                          {canManageAnnouncement(a) && (
                            <>
                              <Link
                                to={`${a.id}/edit`}
                                className="btn btn-outline-warning btn-sm rounded-3 p-2 d-flex shadow-sm"
                              >
                                <Icon name="edit" />
                              </Link>
                              <button
                                onClick={() => handleDelete(a.id)}
                                className="btn btn-outline-danger btn-sm rounded-3 p-2 d-flex shadow-sm"
                              >
                                <Icon name="trash" />
                              </button>
                            </>
      ...
      {!loading && meta && meta.last_page > 1 && (
        <div className="d-flex justify-content-between align-items-center mt-4">
          ...
          <div className="d-flex gap-2">
            <button
              className="btn btn-white border px-4 shadow-sm"
